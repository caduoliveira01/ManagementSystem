        ...
        <form className='space-y-5' onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <FormField control={form.control}
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <Select onValueChange={(value) => { handleTagChange(value); field.onChange(value); }}
                    type="text">
                    <SelectTrigger className="w-full">
                    ...
                    <SelectContent>
                      {tag.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </FormControl>
                <div className='gap-1 flex flex-wrap'>
                  {selectedTags.map((item) => (
                    <div key={item} onClick={() => handleRemoveTag(item)} className='cursor-pointer flex rounded-full items-center border gap-2 py-1 px-4'>
          ...
          />
          <DialogClose>
            {false ? <div><p>You can create only 3 projects with free plan</p></div> :
